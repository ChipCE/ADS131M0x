...
void ADS131M04::sendCommand(uint16_t command)
{
  ...
  delayMicroseconds(1);
#endif
  spiPort->transfer(msb);
  spiPort->transfer(lsb);
...
void ADS131M04::unlockRegisters()
{
  if (isLockSPI())
  {
...
void ADS131M04::begin(SPIClass *port, uint8_t clk_pin, uint8_t miso_pin, uint8_t mosi_pin, uint8_t cs_pin, uint8_t drdy_pin)
{
  ...
  pinMode(drdyPin, INPUT_PULLUP);  // DRDY Input
...
uint16_t ADS131M04::readStatusRegister()
{
  return readRegister(REG_STATUS);
}

uint16_t ADS131M04::readRegisterRaw(uint8_t address)
{
